...
# Function to load dataset with SIFT features
def load_dataset_with_sift(dataset_path):
    X = []
    y = []
    ...
    for label in os.listdir(dataset_path):
        ...
        if os.path.isdir(label_path):
            for file in os.listdir(label_path):
                ...
                if img is not None:
                    img = cv2.medianBlur(img, 5)  # Apply median blur
...
# Load dataset
X, y = load_dataset_with_sift(dataset_path)
...
print("Model training with SIFT features complete and saved.")
...
def real_time_gesture_recognition():
    ...
    while True:
        ...
        gray_frame = cv2.medianBlur(gray_frame, 5)  # Apply median blur
        resized_frame = cv2.resize(gray_frame, (img_size, img_size))

        # Extract SIFT features
        keypoints, descriptors = sift.detectAndCompute(resized_frame, None)
        if descriptors is not None:
